                    ...
                    if source_region == target_region and len(source_ips) > 1:
                        for i, s_ip in enumerate(source_ips):
                            for j, t_ip in enumerate(target_ips):
                                # 避免自己测自己
                                if i != j:
                                    print(f"\nTesting latency within {source_region}: Instance {i+1} -> Instance {j+1} (using {('private' if args.use_private_ip else 'public')} IPs)")
        ...
        if args.source_region == args.target_region and len(instance_data["instances"][args.source_region][ip_type]) > 1 and args.intra_region:
            ...
            for i, s_ip in enumerate(source_ips):
                for j, t_ip in enumerate(target_ips):
                    # 避免自己测自己
                    if i != j:
                        print(f"\nTesting latency within {args.source_region}: Instance {i+1} -> Instance {j+1} (using {('private' if args.use_private_ip else 'public')} IPs)")
                        result_file, ping_stats = run_ping_test(
                            t_ip, s_ip, args.ssh_key, 
                            args.ping_count, args.output_dir
                        )
