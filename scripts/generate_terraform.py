...
def update_variables_tf(regions, output_file, ami_ids):
    ...
    try:
        ...
        ami_ids_entries = []
        for region in regions:
            ami_ids_entries.append(f'    "{region}" = "{ami_ids.get(region, FALLBACK_AMI_IDS.get(region, "ami-0000000000000000"))}" # {get_region_name(region)}')
...
def modify_run_benchmark_py(config_hook_file):
    ...
    try:
        ...
        setup_terraform_func = re.search(r'def setup_terraform\(config\):(.*?)return True', content, re.DOTALL)
...
"""
            new_function = setup_terraform_func.group(0).replace('os.makedirs(data_dir, exist_ok=True)', 'os.makedirs(data_dir, exist_ok=True)' + hook_code)
            content = content.replace(setup_terraform_func.group(0), new_function)
            ...
            print(f"Could not find setup_terraform function in {config_hook_file}")
    except Exception as e:
        print(f"Error modifying run_benchmark.py: {e}")

def main():
    parser = argparse.ArgumentParser(description="Generate Terraform configuration files from config.json")
    parser.add_argument("--config", default="../terraform/config.json", help="Path to config.json file")
