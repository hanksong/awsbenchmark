...
def run_udp_test(instance_info_path, ssh_key_path, output_dir, use_private_ip, server_region, bandwidth, duration):
    ...
    output_file = os.path.join(output_dir, f"udp_results_{timestamp}.csv")

    # Ensure output directory exists
    os.makedirs(output_dir, exist_ok=True)

    # Validate server_region
    if server_region not in instance_info['instances']:
        print(
            f"Error: Specified server region '{server_region}' not found in instance info.")
        # Fallback: Use the first region if available
        if regions:
    ...
    try:
        ...
        print(
            f"warning: unable to start the iperf3 server on {server_ip}: {e}")
...
def main():
    ...
    if len(server_ips) > 1 and args.intra_region:
        for server_idx, server_ip in enumerate(server_ips):
            ...
            print(
                f"server ip: {server_ip} (using {('private' if args.use_private_ip else 'public')})")
