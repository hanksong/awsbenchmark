...
def setup_terraform(config):
    ...
    if os.path.exists(pub_key_path):
        try:
            ...
            temp_key_file = os.path.join("/tmp", f"{key_name}_{int(time.time())}.pub")
            ...
            for region in aws_regions:
                ...
                try:
                    ...
                    print(f"Successfully deleted old key in {region} (if it existed)")
                except:
                ...
                if result and "KeyPairId" in result:
                    ...
                    print(f"First import method failed for {region}, trying alternative method...")
                    ...
                    try:
                        ...
                        print(f"Importing with alternative method to {region}...")
                        temp_key_json = os.path.join("/tmp", f"{key_name}_{region}_key.json")
                        ...
                        if base64_result:
                            ...
                            if result2 and "KeyPairId" in result2:
                                print(f"Alternative method succeeded for {region}!")
                            else:
                                ...
                                if not result3 or "Error" in str(result3):
                                    print(f"WARNING: Failed to import key to {region} after multiple attempts!")
                        ...
                        print(f"Error during alternative key import for {region}: {str(e)}")
                ...
                if verify_result and "KeyPairs" in verify_result and key_name in verify_result:
                    ...
                    print(f"❌ CRITICAL: Key verification failed for {region}! This will cause deployment to fail.")
    ...
    try:
        ...
        for region_name, values in terraform_data["instance_public_ips"]["value"].items():
            ...
            if not aws_region:
                print(f"Warning: Could not map region name {region_name} to AWS region code")
                continue
    ...
    print(f"Terraform configuration applied, instance info saved to: {instance_info_path}")
    return True

def install_iperf3(config):
    """Install iperf3 on all EC2 instances"""
    instance_info_path = os.path.join(PROJECT_ROOT, "data/instance_info.json")
    ssh_key_path = os.path.expanduser(f"~/.ssh/{config.get('ssh_key_name', 'aws-network-benchmark')}")
    
    try:
        with open(instance_info_path, 'r') as f:
            instance_data = json.load(f)
        
        install_script_path = os.path.join(PROJECT_ROOT, "scripts/install_iperf3.sh")
        ...
        for region, info in instance_data['instances'].items():
            for ip in info['public_ips']:
                ...
                if not ip or ip == "":
                    print(f"Warning: Instance in {region} region has no public IP, skipping installation")
                    continue
                    
                print(f"Installing iperf3 on instance {ip} in {region} region...")
...
def process_test_results(config):
    ...
    if not collected_file or not os.path.exists(collected_file):
        ...
        result_files = [f for f in os.listdir(data_dir) if f.startswith("collected_results_")]
        if result_files:
...
def generate_visualizations(result_files, config):
    ...
    if not result_files.get('summary_json'):
        ...
        if summary_files:
            ...
            result_files['summary_json'] = os.path.join(data_dir, latest_summary)
        ...
        for line in output.splitlines():
            if line.startswith("- ") and (".png" in line or ".jpg" in line):
                ...
                if os.path.exists(image_file):
                    ...
                    print(f"Warning: Generated image file not found: {image_file}")
    ...
    if result_files.get('summary_json') and os.path.exists(result_files['summary_json']):
        ...
        if output:
            for line in output.splitlines():
                if "Report generation completed:" in line:
                    report_file = line.split("Report generation completed:")[-1].strip()
                    break
                elif "HTML report generated:" in line:
                    report_file = line.split("HTML report generated:")[-1].strip()
                    break
        ...
        if not result_files.get('summary_json'):
            ...
            print(f"Warning: Summary file does not exist: {result_files['summary_json']}")
...
def main():
    parser = argparse.ArgumentParser(description="AWS Network Benchmark Automation Script")
    parser.add_argument("--config", default="../config.json", help="Configuration file path")
    parser.add_argument("--skip-terraform", action="store_true", help="Skip Terraform deployment step")
    parser.add_argument("--skip-install", action="store_true", help="Skip iperf3 installation step")
    parser.add_argument("--skip-tests", action="store_true", help="Skip network tests step")
    parser.add_argument("--cleanup", action="store_true", help="Clean up AWS resources after testing")
    ...
    print(f"AWS Network Benchmark starting, results will be saved to: {run_dir}")
